...
// ── Drop preview ghost ─────────────────────────────────────────
function DropGhost({ y, height, visible }: { y: number; height: number; visible: boolean }) {
...
// ── Resize handle ─────────────────────────────────────────────
function ResizeHandle({ blockId, startY, blockEndY }: { blockId: string; startY: number; blockEndY: number }) {
  ...
  const onPointerDown = useCallback((e: React.PointerEvent) => {
    e.stopPropagation();
    e.preventDefault();
    const el = (e.target as HTMLElement).closest(".day-col-grid") as HTMLElement | null;
    if (!el) return;
    ...
    const onMove   = (ev: PointerEvent) => {
      ...
      rafRef.current = requestAnimationFrame(() => {
        ...
        usePlannerStore.getState().resizeBlock(blockId, newEnd);
      });
    };
    const onUp = () => {
      if (rafRef.current) cancelAnimationFrame(rafRef.current);
  ...
  return (
    <div
      ...
      className="absolute bottom-0 left-0 right-0 h-2.5 cursor-ns-resize flex items-center justify-center opacity-0 group-hover:opacity-100 transition-fast"
    >
...
export function DayColumn({ date, compact = false }: { date: ISODate; compact?: boolean }) {
  ...
  // ── Pointer drag-over (from TaskSidebar) ──────────────────────
  ...
  // ── Block drag-move (move existing block) ─────────────────────
  const startBlockDrag = useCallback((e: React.PointerEvent, block: TimeBlock) => {
    ...
    const onMove = (ev: PointerEvent) => {
      ...
      // Optimistic update for smooth drag feel
  ...
  return (
    <div className="flex flex-col flex-1 min-w-0">
      ...
      >
        ...
        {/* Drop ghost preview */}
        {ghost && <DropGhost y={ghost.y} height={ghost.height} visible />}
...
function BlockCard({
  ...
  return (
    ...
    >
      ...
      {/* Resize handle */}
      <ResizeHandle blockId={block.id} startY={top} blockEndY={top + height} />
    </div>
  );
}

function ResizeHandle({ blockId, startY }: { blockId: string; startY: number; blockEndY: number }) {
  const { resizeBlock } = usePlannerStore();
  const rafRef = useRef<number | null>(null);
  ...
  const onPointerDown = useCallback((e: React.PointerEvent) => {
    e.stopPropagation();
    e.preventDefault();
    const el = document.querySelector(".day-col-grid") as HTMLElement | null;
    if (!el) return;
